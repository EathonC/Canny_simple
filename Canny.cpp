...
void Canny2(InputArray _src, OutputArray _dst,double low_thresh, double high_thresh,int aperture_size = 3, bool L2gradient = false)
{
	...
	int maxsize = max(1 << 10, src.cols * src.rows / 10); // 2的10次幂 1024  
	vector<uchar*> stack(maxsize); // 定义指针类型向量，用于存地址，共有maxsize个空间
	//以vector为底层构建stack的优点：可访问栈底，pop时可返回值，提供迭代器操作（可以随意访问任意元素），所占内存空间可分配（提高性能？减少不必要的性能损失）
		...
		if (i < src.rows)
		{
			...
			if (!L2gradient) // 如果 L2gradient为false  
			{
				for (int j = 0; j < src.cols*cn; j++) // 对第i行里的每一个值都进行计算  
					_norm[j] = abs(int(_dx[j])) + abs(int(_dy[j])); // 用||+||计算  
			}
			else
			{
...
#define CANNY_SHIFT 15  
			// tan22.5  
			const int TG22 = (int)(0.414*(1 << CANNY_SHIFT) + 0.5);
...
int main()
{
    Mat dst, edge, gray, dst1, gray1, edge1;
	Mat src = imread("test.png");
	Mat src1 = src.clone();
	Mat src2 = src.clone();
	dst.create(src1.size(), src1.type());//创建矩阵
	dst1.create(src2.size(), src2.type());
	imshow("原图", src);
	cvtColor(src1, gray, COLOR_BGR2GRAY);//转为灰度图像
	//imshow("灰度", gray);
	cvtColor(src2, gray1, COLOR_BGR2GRAY);
	GaussianBlur(gray, edge, Size(3, 3), 0, 0);//3*3内核降噪
	//imshow("高斯滤波", edge);
	GaussianBlur(gray1, edge1, Size(3, 3), 0, 0);
	Canny2(edge, edge, 3, 9, 3);
	Canny(edge1, dst1, 3, 9, 3);
	dst = Scalar::all(0);
	src1.copyTo(dst, edge);//原图.copyTo（效果图，边缘）
	imshow("my function", edge);
	imshow("system function", dst1);
	waitKey();
	//src.release();
}
